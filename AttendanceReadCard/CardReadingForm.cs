...
namespace AttendanceReadCard
{
    ...
    {
        public CardReadingForm(CardSetup setup)
        {
            InitializeComponent();
            CardSetup = setup;
        }
        ...
        private void ReadingTask_DoWork(object sender, DoWorkEventArgs e)
        {
            try
            {
                OMRCardReader.Open(35, 50); //看卡片規格。
                ...
                while (true)
                {
                    ...
                    {
                        XElement phrase1 = OMRGushanParser.Parser(data);

                        XElement errors;
                        if (!OMRGushanParser.IsValidated(phrase1, out errors))
                        {
                            e.Result = errors;
                            break;
                        }

                        XElement phrase2 = CardSetup.TransformData(phrase1);
        ...
        private void ReadingTask_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            else //有錯!!
            {
                ...
                if (e.Result is XElement)
                {
                    ...
                    foreach (XElement each in (e.Result as XElement).Elements("Msg"))
                        msg += each.Value + "\n";
                    msg += "\n繼續讀卡？";
